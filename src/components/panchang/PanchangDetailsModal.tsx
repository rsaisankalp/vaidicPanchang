...
interface PanchangDetailsModalProps {
  isOpen: boolean;
  onClose: () => void;
  originalSelectedDate: Date | undefined; // Date object user clicked
...
export function PanchangDetailsModal({
  ...
  const renderDetails = () => {
    if (isLoading) {
      return <div className="min-h-[400px] flex flex-col items-center justify-center"><div className="loader ease-linear rounded-full border-4 border-t-4 border-muted h-12 w-12 mb-4 animate-spin border-t-primary"></div><p className="text-muted-foreground">Loading details...</p></div>;
    }
    if (!details) {
